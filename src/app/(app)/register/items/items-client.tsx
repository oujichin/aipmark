...
interface ItemsClientProps {
  ...
  initialCategories: MasterRecord[];
  initialFields: DataFieldMasterRecord[];
  processId: string | null;
}

export default function ItemsClient({ initialItems, initialCategories, initialFields, processId }: ItemsClientProps) {
  const [items, setItems] = useState<RegisterItem[]>(initialItems);
  const [categories] = useState<MasterRecord[]>(initialCategories);
  const [fields] = useState<DataFieldMasterRecord[]>(initialFields);
  ...
  async function handleRefresh() {
    ...
    const url = processId
      ...
      : "/api/register/items";
    try {
      const [itemRows, categoryRows, fieldRows] = await Promise.all([
        fetch(url).then((r) => r.json()),
        fetch("/api/master/data-categories").then((r) => r.json()),
        fetch("/api/master/data-fields").then((r) => r.json()),
      ]);
      setItems(Array.isArray(itemRows) ? itemRows : itemRows.items ?? []);
      // Categories and fields are master data, unlikely to change but update if needed
      void categoryRows;
      void fieldRows;
    } finally {
      setLoading(false);
  ...
  // Keep handleRefresh available for future use
  void handleRefresh;
