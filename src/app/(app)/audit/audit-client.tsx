...
export default function AuditClient({ initialPlans, initialCorrectives }: AuditClientProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (plansRes.ok) {
        ...
        const plansData: AuditPlan[] = Array.isArray(plansRaw) ? plansRaw : plansRaw.items ?? [];
        setPlans(plansData);
      }
      if (correctivesRes.ok) {
        const correctivesRaw = await correctivesRes.json();
        const correctivesData: CorrectiveAction[] = Array.isArray(correctivesRaw) ? correctivesRaw : correctivesRaw.items ?? [];
  ...
  const findings: AuditFinding[] = plans.flatMap((plan) =>
    plan.findings.map((f) => ({
      ...f,
      auditPlan: { id: plan.id, title: plan.title },
    }))
  );
