...
export interface ReviewsClientProps {
  initialReviews: ManagementReview[];
...
export default function ReviewsClient({ initialReviews, initialUsers }: ReviewsClientProps) {
  const [reviews, setReviews] = useState<ManagementReview[]>(initialReviews);
  const [selectedReview, setSelectedReview] = useState<ManagementReview | null>(null);
  const [users] = useState<OrgUser[]>(initialUsers);
  ...
  const fetchReviews = useCallback(async () => {
    try {
      ...
      const data: ManagementReview[] = Array.isArray(raw) ? raw : raw.items ?? [];
      setReviews(data);
    } catch (e) {
  ...
  // initialReviewsを使うので初回fetchは不要
  // usersもinitialUsersを使う
