...
import { interviewHypothesesSchema } from "@/lib/validations/ai";
...
export async function POST(req: NextRequest) {
  ...
  const parsed = parseBody(interviewHypothesesSchema, body);
  if (!parsed.success) return parsed.error;
  const { companyName, profile, processName, processDescription } = parsed.data;
  ...
  try {
    ...
    console.error("POST /api/ai/interview-hypotheses error:", error);
